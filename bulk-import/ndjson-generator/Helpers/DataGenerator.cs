namespace NDJsonGenerator.Helpers
{
    using System;
    ...
    public class DataGenerator
    {
        ...
        public static IEnumerable<string> GenerateModels(int numModels)
        {
            ...
            {
                string model = string.Format(ModelsTemplate, i, (i + 1) % numModels);
                models.Add(model);
            }
        ...
        public static IEnumerable<BasicDigitalTwin> GenerateTwins(int numModels, int numTwins)
        {
            ...
            for (var i = 0; i < numTwins; i++)
            {
                ...
                twins.Add(JsonSerializer.Deserialize<BasicDigitalTwin>(twin));
            }
        ...
        {
            List<BasicRelationship> relationships = new List<BasicRelationship>();
            for (var i = 0; i < numRelationships; i++)
            {
                string relationship = string.Format(RelationshipsTemplate, i, (i + 1) % numRelationships);
                relationships.Add(JsonSerializer.Deserialize<BasicRelationship>(relationship));
            }
